...
to_batch_nu = 91
run_analysis = True
name1 = 'dfB{}T{}'.format(from_batch_nu,to_batch_nu-1)
...
def SPheno_reader(i,j,SPheno_spc_File):
    ...
    data_spheno2 ='batch_{}/Result_data/Spheno_output_Folder/point_{}/LesHouches.in.BGLNCS'.format(i,j,SPheno_spc_File)
    ImR = open(data_spheno2, 'r')
    for Imline in ImR:
        if Imline.startswith('Block IMMINPAR'):Pass5 = True
        if Imline.startswith('Block SPhenoInput'):Pass5 =False
        if Pass5:
            Imcouplings.append(Imline.strip('\n'))
    ...
    imY1u11,imY1u12,imY1u21,imY1u22,imY2u33=float(Imcouplings[10].split()[1]),float(Imcouplings[11].split()[1]),float(Imcouplings[12].split()[1]),    float(Imcouplings[13].split()[1]),float(Imcouplings[14].split()[1]) 
    imY1e11,imY1e12,imY1e21,imY1e22,imY2e33 =float(Imcouplings[15].split()[1]),float(Imcouplings[16].split()[1]),float(Imcouplings[17].split()[1]),    float(Imcouplings[18].split()[1]),float(Imcouplings[19].split()[1])
    ...
    SphenoDataList = [i,j,Lambda1,Lambda2,Lambda3,Lambda4,Lambda1Dash,Lambda2Dash,Lambda3Dash,Mu3,Mub,alpha1,alpha2,alpha3,alpha4,    Y1d11,Y1d12,Y1d13,Y1d21,Y1d22,Y1d23,Y2d31,Y2d32,Y2d33,Y1u11,Y1u12,Y1u21,Y1u22,Y2u33,Y1e11,Y1e12,Y1e22,Y1e33,    Y1n11,Y1n12,Y1n22,Y1n33,B11,B12,B21,B22,C13,C23,C31,C32,    imY1d11,imY1d12,imY1d13,imY1d21,imY1d22,imY1d23,imY2d31,imY2d32,imY2d33,imY1u11,imY1u12,imY1u21,imY1u22,imY2u33,imY1e11,imY1e12,imY1e21,imY1e22,imY2e33,    V1,V2,VS,Mhh1,Mhh2,Mhh3,MAh2,MAh3,MHm2,MFv1,MFv2,MFv3,MFv4,MFv5,MFv6,    T_parameter,S_parameter,U_parameter]
...
def Rangles_function(i,j):
    data = 'batch_{}/Result_data/Spheno_output_Folder/point_{}/input_LHE.BGLNCS'.format(i,j)
    R = open( data, 'r')
    for line in R:
        if not line.lstrip().startswith('#'):
            if line.lstrip().startswith('beta'):beta = float(line.split()[1])
            if line.lstrip().startswith('a2'):a2 = float(line.split()[1])
            if line.lstrip().startswith('a3'):a3 = float(line.split()[1])
            if line.lstrip().startswith('delta'):delta = float(line.split()[1])
            if line.lstrip().startswith('gamma1'):gamma1 = float(line.split()[1])
    #return angles
    Langles = [beta,a2,a3,delta,gamma1]
    return Langles
...
def Read_MG(i,j):
    data = 'batch_{}/Result_data/Spheno_output_Folder/point_{}/MGdata.txt'.format(i,j)
    R = open( data, 'r')
    for line in R:
        line.split(",")
    return line.split(",")
...
if run_analysis:
    for i in range(from_batch_nu,to_batch_nu):
        ...
        for j in range(1,number_points+1):
            ...
            MGdata = Read_MG(i,j)
            NewDatalist = SphenoDataList + HBHS + RatioList + Langles + MGdata
            WdataSPheno(name1,NewDatalist)
